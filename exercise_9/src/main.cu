...
__global__ void
simpleStencil_Kernel(StencilArray_t d_array)
{
    // index of array
    int iIndex = blockIdx.x * blockDim.x + threadIdx.x;

    // value of actual element
    float fThisValue = d_array.array[iIndex];

    float act_sum;

    if(!isBorder(d_array.size, iIndex)) {

        act_sum = fThisValue + 24.0 / 100.0
                  * ((-4.0) * fThisValue
                  + d_array.array[iIndex + 1]
                  + d_array.array[iIndex - 1]
                  + d_array.array[iIndex + d_array.size]
                  + d_array.array[iIndex - d_array.size]);
...
__global__ void
optStencil_Kernel(StencilArray_t d_array)
{
    ...
    shMemBlock[threadIdx.x] = fThisValue;
    ...
    if(!isBorder(d_array.size, iIndex)) {
        ...
        act_sum = fThisValue + 24.0 / 100.0
                  * ((-4.0) * fThisValue
                  + shMemBlock[threadIdx.x + 1]
                  + shMemBlock[threadIdx.x - 1]
                  + shMemBlock[threadIdx.x + d_array.size]
                  + shMemBlock[threadIdx.x - d_array.size]);
...
//  srand(static_cast<unsigned>(time(0)));
    ...
    cudaMemcpy(d_array.array, h_array.array,
            static_cast<size_t>(numElements * sizeof(*d_array.array)),
    ...
    float shMemSize = blockSize * sizeof(float);

    kernelTimer.start();

    for (int i = 0; i < numIterations; i ++) {
#ifndef _OPT_KERNEL
		simpleStencil_Kernel<<<grid_dim, block_dim>>>(d_array);
#else
        optStencil_Kernel<<<grid_dim, block_dim, shMemSize>>>(d_array);
#endif
    }
